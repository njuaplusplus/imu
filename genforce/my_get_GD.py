...
def parse_args(model_name, num, batch_size, trunc_psi=0.7, trunc_layers=8, randomize_noise=False, seed=0):
    """Parses arguments."""
    ...
    parser.add_argument('--trunc_layers', type=int, default=trunc_layers,
                        help='Number of layers to perform truncation. This is '
                             'particularly applicable to StyleGAN (v1/v2). '
                             '(default: %(default)s)')
    parser.add_argument('--randomize_noise', type=bool_parser, default=randomize_noise,
...
def main(device, model_name, num, batch_size, use_w_space=True, use_discri=True, repeat_w=True, use_z_plus_space=False, trunc_psi=0.7, trunc_layers=8, randomize_noise=False, seed=0):
    """Main function."""
    args = parse_args(model_name, num, batch_size, trunc_psi, trunc_layers, randomize_noise=randomize_noise, seed=seed)
    print(args)
    if args.num <= 0:
        return
    ...
    def fake_generator(code, randomize_noises=None, special_noises=None, return_all_results=False):
...
class Fake_G:
    ...
    def __call__(self, code, randomize_noises=None, special_noises=None, return_all_results=False):
        # print(f'code.shape {code.shape}')
        return self.g_function(code, randomize_noises=randomize_noises, special_noises=special_noises, return_all_results=return_all_results)
