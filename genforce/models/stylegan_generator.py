...
class StyleGANGenerator(nn.Module):
    ...
    def forward(self,
        ...
        synthesis_results = self.synthesis(wp, lod, randomize_noise, randomize_noises, special_noises, return_noise)
...
class MappingModule(nn.Module):
    ...
    def __init__(self,
        ...
        if label_size:
            ...
            self.pth_to_tf_var_mapping['label_weight'] = 'LabelConcat/weight'
...
class SynthesisModule(nn.Module):
    ...
    def forward(self, wp, lod=None, randomize_noise=False,
                randomize_noises=None, special_noises=None, return_noise=False, special_styles=None):
        ...
        if randomize_noises is None or len(randomize_noises) == 0:
            randomize_noises = [False] * ((self.final_res_log2 + 1 - self.init_res_log2) * 2)
        ...
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            ...
            if lod < current_lod + 1:
                ...
                if block_idx == 0:
                    if self.const_input:
                        x, *style = self.layer0(None, wp[:, 0], randomize_noise or randomize_noises[block_idx], special_noises[block_idx], return_noise)
                    else:
                        x = wp[:, 0].view(-1, self.w_space_dim, 1, 1)
                        x, *style = self.layer0(x, wp[:, 0], randomize_noise or randomize_noises[block_idx], special_noises[block_idx], return_noise)
                else:
                    x, *style = self.__getattr__(f'layer{2 * block_idx}')(
                        x, wp[:, 2 * block_idx], randomize_noises[2*block_idx], special_noises[2*block_idx], return_noise)
                if return_noise:
                    style, noise, x0_mean, x0_std = style
                    results[f'stochastic_noise{2*block_idx:02d}'] = noise.detach().clone()
                    results[f'x0_mean{2 * block_idx:02d}'] = x0_mean.detach().clone()
                    results[f'x0_std{2 * block_idx:02d}'] = x0_std.detach().clone()
                else:
                    style, x0_mean, x0_std = style
                    results[f'x0_mean{2 * block_idx:02d}'] = x0_mean.detach().clone()
                    results[f'x0_std{2 * block_idx:02d}'] = x0_std.detach().clone()

                results[f'style{2 * block_idx:02d}'] = style
                x, *style = self.__getattr__(f'layer{2 * block_idx + 1}')(
                    x, wp[:, 2 * block_idx + 1], randomize_noises[2*block_idx+1], special_noises[2*block_idx+1], return_noise)
                if return_noise:
                    style, noise, x0_mean, x0_std = style
                    results[f'stochastic_noise{2*block_idx+1:02d}'] = noise.detach().clone()
                    results[f'x0_mean{2 * block_idx+1:02d}'] = x0_mean.detach().clone()
                    results[f'x0_std{2 * block_idx+1:02d}'] = x0_std.detach().clone()
                else:
                    style, x0_mean, x0_std = style
                    results[f'x0_mean{2 * block_idx+1:02d}'] = x0_mean.detach().clone()
                    results[f'x0_std{2 * block_idx+1:02d}'] = x0_std.detach().clone()
...
class NoiseApplyingLayer(nn.Module):
    ...
    def forward(self, x, randomize_noise=False, special_noises=None, return_noise=False):
        if x.ndim != 4:
            raise ValueError(f'The input tensor should be with shape '
                             f'[batch_size, channel, height, width], '
                             f'but `{x.shape}` is received!')
        if special_noises is not None:
            noise = special_noises
        else:
            if randomize_noise:
                # print('randomize_noise is true', self.res)
                noise = torch.randn(x.shape[0], 1, self.res, self.res).to(x)
            else:
                noise = self.noise
        if return_noise:
            return x + noise * self.weight.view(1, -1, 1, 1), noise
        else:
...
class ConvBlock(nn.Module):
    ...
    def forward(self, x, w, randomize_noise=False, special_noises=None, return_noise=False):
        if self.position != 'const_init':
            x = self.upsample(x)
        ...
        x = self.apply_noise(x, randomize_noise, special_noises, return_noise)
        if return_noise:
            x, noise = x
        ...
        if bias is not None:
            x = x + bias.view(1, -1, 1, 1)
        x = self.activate(x)
        x, x0_mean, x0_std = self.normalize(x)
        x, style = self.style(x, w)
        if return_noise:
            return x, style, noise, x0_mean, x0_std
        else:
            return x, style, x0_mean, x0_std
